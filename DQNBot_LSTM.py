...
class DQNBot(gym.core.Env):
    def __init__(self):
        ...
        self.con = sqlite3.connect('board_1.db')
        self.cur = self.con.cursor()
        
        df_low = psql.read_sql('SELECT 0, min(mid_price), min(ask_price_100), min(ask_price_200), min(ask_price_300), min(ask_price_500), min(ask_price_800), min(ask_price_1300), min(ask_price_2100), min(ask_price_3400), min(ask_price_5500), min(ask_price_8900), \
        ...
        df_high = psql.read_sql('SELECT 1, max(mid_price), max(ask_price_100), max(ask_price_200), max(ask_price_300), max(ask_price_500), max(ask_price_800), max(ask_price_1300), max(ask_price_2100), max(ask_price_3400), max(ask_price_5500), max(ask_price_8900), \
...
if __name__ == "__main__":
    ...
    # DQNのネットワーク定義
    model = Sequential()
    model.add(LSTM(units=12, return_sequences=False, input_shape=input_shape))
    ...
    if sys.argv[1] == 'train':
        ...
        history = dqn.fit(env, nb_steps=3600, visualize=False, verbose=2, nb_max_episode_steps=1200)
        dqn.save_weights('dqnbot.h5')
    
    elif sys.argv[1] == 'test':
        dqn.load_weights('dqnbot.h5')
